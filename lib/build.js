 ...
 * @property {Array<string>} [rootCopy] Array of paths to be copied to output root
...
const buildSteps = {
  ...
  rootCopy: require('./bulid-steps/root-copy')
};
...
const steps = [
  ...
  {
    name: 'Copy files',
    pre: () => log.yellow('Copying files...'),
    action: buildSteps.rootCopy,
    post: () => log.green('✔ Files copied', -1)
  }
]
...
async function build(config) {
  ...
  if (config.rootCopy) {
    let newRootCopy = [];
    for (let copyPath of config.rootCopy) {
      newRootCopy.push( absolutify(copyPath) );
    }
    config.rootCopy = newRootCopy;
  }
