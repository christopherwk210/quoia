...
async function buildScripts(config) {
  ...
  for (let script of pageGlobalScripts) {
    let scriptsContents;
    try {
      scriptsContents = await fsExt.quickRead(script);
    } catch(e) {
      throw new Error(e);
    }
  ...
  // Apply transform
  loadedScripts = await config.scriptTransform(builtScript, currentComponent.build.directory);
...
async function loadComponentScripts(component) {
  ...
  for (let script of externalScripts) {
    let scriptContents;
    try {
      scriptContents = await fsExt.quickRead(script);
    } catch(e) {
      throw new Error(e);
    }
